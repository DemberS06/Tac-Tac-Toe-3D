    ...
    while (K--) {
        ...
        for (auto& [board, turno] : Seeds) { // Itera sobre las semillas guardadas
            ...
            for (const auto& [u, MovesForState] : Ans.first) {
                ...
                if (MovesForState.first.empty()) { // Si no hay movimientos óptimos registrados
                    for(i=0; i<26; ++i) fout << "0 "; // Imprime ceros como placeholder
                } else {
                    // Imprime el vector de movimientos tal cual está en el mapa
                    for (const auto& w : MovesForState.first) fout << w << ' ';
                }
                fout << MovesForState.second << "\n";
            }
            Ans.first.clear();
        }
    }
...
void Struct_Data() {
    ...
    int i, N, M, K, new_Value; // Contadores y valor 'best' (parece indicar si hay mov. ganador)
    double best, worse, rate;
    SeedAns_t Ans; // Mapa para almacenar los movimientos combinados para cada ID de tablero
                    // Clave: ID del tablero. Valor: vector<int> representando ¿profundidad/score? para cada posible casilla (0-25).
    vector<int> new_Distance(26); // Vector temporal para leer los movimientos/distancias de una línea del archivo
    ...
    while (K--) {
        ...
        while (N--) {
            ...
            while (M--) {
                ...
                for (auto& u : new_Distance) { // Lee 26 valores en el vector temporal 'Distance'
                    rin >> u;  
                }
                ...
                auto& [Distance, Value] = Ans[id]; // Obtiene referencia al vector asociado a este ID
                
                if (Distance.empty()) { // Si es la primera vez que vemos este ID...
                    Distance.resize(26, 0); // ...lo redimensiona a 26 y lo inicializa con ceros.
                    Value=-2;
                }
                ...
                if(Value == 1){
                    for(i = 0; i < 26; i++){
                        if(Distance[i] == 0)Distance[i] = new_Distance[i];
                        else Distance[i] = min(Distance[i], new_Distance[i]);
                    }
                }
                else{
                    for(i = 0; i < 26; i++){
                        Distance[i] = max(Distance[i], new_Distance[i]);
                    }
                }
            }
        }
    ...
    auto Print_Data = [& rate, & best, & worse, & id, & i](SeedAns_t& DataStructure, string Direction){
        ofstream rout(Direction);
        if (!rout.is_open()) {
            cerr << "Error: No se pudo abrir " << Direction << " para escritura" << endl;
            rout.close();
            return;
        }

        // Escribe la cabecera de la base de datos estructurada (cantidad de tableros)
        rout << DataStructure.size() << "\n";

        // Escribe cada entrada de la base de datos estructurada
        for (auto& [u, v] : DataStructure) { // Itera sobre el mapa filtrado
            ...
            for(auto &w: Distance){
                if(w == 0)rout << "-1 ";
                else if(w == best) rout << "1 ";
                else{
                    rate = (worse - w + 0.00000001) / (worse - best + 0.00000001) / 2;
                    rout << rate << ' ';
                }
            }
        ...
        // Cierra los archivos
        rout.close();
        cout << "Base de datos reestructurada en " << Direction << "\n";
        return;
    };
    ...
    for (auto& [u, v] : Ans) { // Itera sobre el mapa combinado
        id = 0; // Usa 'id' como contador de movimientos no cero
        for (const auto& w : v.first) { // Itera sobre el vector de valores para este tablero
    ...
    Print_Data(Ans, "Base_de_Datos.txt");

    SeedAns_t Ans2 = Ans;

    // Filtra los tableros Para el jugador 1
    trash.clear(); // Vector para guardar IDs a eliminar
    for (auto& [u, v] : Ans) { // Itera sobre el mapa combinado
        id = 0; // Usa 'id' como contador de movimientos no cero
        for (long long w=u; w>0; w/=3) { // Itera sobre el vector de valores para este tablero
            id += (w%3!=0); // Cuenta cuántos valores son distintos de cero
        }
        if (id&1) { // Si todos los valores son cero...
            trash.push_back(u); // ...marca este ID para eliminar.
        }
    }
    // Elimina los IDs marcados del mapa 'Move'
    ...
    // Filtra los tableros para el jugador 2
    trash.clear(); // Vector para guardar IDs a eliminar
    for (auto& [u, v] : Ans2) { // Itera sobre el mapa combinado
        id = 0; // Usa 'id' como contador de movimientos no cero
        for (long long w=u; w>0; w/=3) { // Itera sobre el vector de valores para este tablero
            id += (w%3!=0); // Cuenta cuántos valores son distintos de cero
        }
        if (id%2==0) { // Si todos los valores son cero...
            trash.push_back(u); // ...marca este ID para eliminar.
        }
    }
    // Elimina los IDs marcados del mapa 'Move'
    ...
    Print_Data(Ans, "Base_de_Datos1.txt");
    Ans.clear();
    Print_Data(Ans2, "Base_de_Datos2.txt");
    Ans2.clear();
}
...
void Compress_Data(){
    cout << "Ingrese la direccion del archivo donde está guardada la Base de Datos:\n"<< "Ejemplo: Base_de_Datos2.txt\n";
    ...
    if (!fin.is_open()) {
        cerr << "Error: No se pudo abrir " << s << endl;
        return;
    } 

    vector<pair<vector<int>, vector<double>>> Move[27], Data;
    vector<int> board(26);
    vector<double> weight(26);

    int n, moves;

    fin>>n;

    cout<<"En total hay "<<n<<" Tableros\n";
    ...
    while(n--){
        ...
        for(auto& u:weight)fin>>u;

        Move[moves].push_back({board, weight});
    }
    ...
    for(int i=0; i<27; i++){
        cout<<"Hay "<<Move[i].size()<<" Tableros con "<<i<<" movimientos hechos\n";
        
        shuffle(Move[i].begin(), Move[i].end(), g);
        
        do{
            cout<<"Ingrese la cantidad a conservar: ";
            cin>>moves;
        }while(moves<0);


        for(int h=0; h<moves; h++)Data.push_back(Move[i][h%Move[i].size()]);
    }

    fout<<Data.size()<<"\n";

    for(auto& [w,v]:Data){
        for(auto& u:w)fout<<u<<' '; fout<<"\n";
        for(auto& u:v)fout<<u<<' '; fout<<"\n";
    }

    fout.close();
    cout << "Base de datos reestructurada en "<<s<<"\n";
}
...
void menu() {
    ...
    case 3:
        ...
        break;
    case 4:
        Compress_Data(); // Llama a la función para reestructurar la BD
        break;
