...
void Testing(){
    ...
    Players.resize(tam); // Ajusta el tamaño del vector de IAs

    // Carga cada IA desde su archivo correspondiente
    for(int i=0; i<tam; i++) {
        cout << "\n--- Cargando IA " << i+1 << " ---\n";
        Players[i].Read_IA(); // Llama al método Read_IA para cada objeto IA
    }
    ...
    for(int i = 0; i < Players.size(); i++){ // IA i
        for(int j = 0; j < Players.size(); j++){ // IA j
            if(j == i) continue; // Una IA no juega contra sí misma
    ...
    for(const auto &u : Players){ // Itera sobre las IAs
        // Imprime en consola
