                         ...
                         const vector<int>& Order, const vector<long long>& Pot_3)
{
    ...
    for (int u : pos) { // u es índice 0-based.
        ...
        if (child_outcome < 0) {
            // ...entonces YO GANO forzando este movimiento.
            Distance[id] = -child_outcome; // Mi resultado es positivo.
            Move[id] = {u}; // Guarda este movimiento ganador.
            board[u] = 0; // Deshace el movimiento antes de retornar.
            return; // Estrategia: Retorna al encontrar la primera victoria forzada.
        }

        board[u] = 0; // Deshace el movimiento si no llevó a una victoria forzada.
    }

    // Si no se encontró Victoria Forzada: Buscar Mejor Derrota (Juego más largo).
    int max_opponent_distance = 0; // Guarda la máxima distancia (>=0) encontrada en hijos.

    // Calcula la máxima distancia encontrada entre los hijos (acceso directo).
    for (int u : pos) { // u es índice 0-based.
        long long new_id = id + player * Pot_3[u];
        // Acceso directo a Distance[new_id]. Si no existe, es 0. std::max(0,x) funciona.
        max_opponent_distance = max(max_opponent_distance, Distance[new_id]);
    }
