...
void IA_t::GetTrain(const vector<long double>& input, const vector<long double>& target) {
    ...
    // Suma los gradientes calculados para ESTE ejemplo a los acumuladores
    // Verifica tamaños por seguridad
    ...
    for (size_t l = 0; l < weights.size(); l++) {
        ...
        for (size_t i = 0; i < layers[l + 1]; i++) {
            ...
            for (size_t j = 0; j < layers[l]; j++) {
                // Gradiente del peso w_lij = delta_i * activacion_j
                GradientWeights[l][i][j] += deltas[l][i] * activations[l][j];
            }
            // Gradiente del bias b_li = delta_i
...
void IA_t::Train(vector<pair<vector<long double>, vector<long double>>>& Seeds) {
    ...
    while (Epochs < 2000) {
        ...
        Learning_Rate = 0.1L / (long double)(int((Epochs + 199) / 200));

        // Baraja el conjunto de datos para esta época
        shuffle(Seeds.begin(), Seeds.end(), g);

        int current_batch_size = 0; // Contador para el tamaño del lote actual
        Restart(); // Pone a cero los gradientes al inicio del lote

        // Itera sobre el conjunto de datos barajado
        for (size_t i = 0; i < Seeds.size(); i++) {
            // Calcula y acumula gradientes para el ejemplo actual
            GetTrain(Seeds[i].first, Seeds[i].second); // Llama al método privado
            current_batch_size++; // Incrementa contador del lote

            // Si el lote está completo O es el último ejemplo del dataset...
            if (current_batch_size == MiniBatchSize || i == Seeds.size() - 1) {
                // ...actualiza los pesos de la red usando los gradientes acumulados
                Update(current_batch_size); // Llama al método privado
                // Pone a cero los acumuladores para el siguiente lote
                Restart();
                // Resetea el contador del tamaño del lote
                current_batch_size = 0;
            }
        } // Fin del bucle sobre el dataset
        ...
        if (Epochs % 50 == 0) { // Guarda en época 50, 100, 150...
