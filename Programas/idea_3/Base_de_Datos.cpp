...
void Struct_Data() {
    ...
    ofstream out1("Base_de_Datos1.txt");

    if (!out1.is_open()) {
        cerr << "Error: No se pudo abrir Base_de_Datos1.txt para escritura" << endl;
        out1.close();
        return;
    }
    ...
    for (auto& [u, v] : Move) { // Itera sobre el mapa filtrado
        ...
        if (best > 0) { // Si el movimiento óptimo es ganador (positivo)
            for (const auto& w : v) {
                if (w == best) out1 << "1 ";    // Movimiento óptimo ganador
                else if (w > 0) out1 << "0.3 "; // Otro movimiento ganador (menos óptimo)
                else if (w == 0) out1 << "0 ";   // Movimiento no considerado/disponible
                else out1 << "-1 ";               // Movimiento perdedor
            }
        } else { // Si el movimiento óptimo es perdedor o empate (best <= 0)
            for (const auto& w : v) {
                // Marca con 1 solo los movimientos que igualan al 'best' (el mejor entre los malos)
    ...
    out1.close();
    cout << "Base de datos reestructurada en Base_de_Datos1.txt\n";

    // Filtra los tableros para el jugador 2
    trash.clear(); // Vector para guardar IDs a eliminar
    for (auto& [u, v] : Move2) { // Itera sobre el mapa combinado
        id = 0; // Usa 'id' como contador de movimientos no cero
        for (long long w=u; w>0; w/=3) { // Itera sobre el vector de valores para este tablero
            id += (w%3!=0); // Cuenta cuántos valores son distintos de cero
        }
    ...
    ofstream out2("Base_de_Datos2.txt");
    
    if (!out2.is_open()) {
        cerr << "Error: No se pudo abrir Base_de_Datos2.txt para escritura" << endl;
        out2.close();
        return;
    }
    ...
    // Escribe cada entrada de la base de datos estructurada
    for (auto& [u, v] : Move2) { // Itera sobre el mapa filtrado
        // Imprime el ID del tablero 'u' en formato base 3
        id = u;
        for (i = 0; i < 26; i++, id /= 3) {
            out2 << id % 3 << ' ';
        ...
        if (best > 0) { // Si el movimiento óptimo es ganador (positivo)
            for (const auto& w : v) {
                if (w == best) out2 << "1 ";    // Movimiento óptimo ganador
                else if (w > 0) out2 << "0.3 "; // Otro movimiento ganador (menos óptimo)
                else if (w == 0) out2 << "0 ";   // Movimiento no considerado/disponible
                else out2 << "-1 ";               // Movimiento perdedor
            }
        } else { // Si el movimiento óptimo es perdedor o empate (best <= 0)
            for (const auto& w : v) {
                // Marca con 1 solo los movimientos que igualan al 'best' (el mejor entre los malos)
    ...
    rin.close();
    out2.close();
    cout << "Base de datos reestructurada en Base_de_Datos2.txt\n";
}
