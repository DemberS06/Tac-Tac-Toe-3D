...
map<long long, int> Winner;         // Almacena quién gana en un estado del tablero
...
void backtracking(int p, vector<int> v, long long id){// Revisa quién gana para un tablero dado utilizando backtracking
    Visited[id]=1;      // Marca el estado actual como visitado
    boards++;           // Incrementa la cantidad de tableros revisados
    ...
    if(pos.empty())return Winner[id]=2, Distance[id]=p, void();  // Si no hay movimientos válidos, se asigna al jugador 2 (empate favorable para este) y se registra la profundidad p
    ...
    for(auto &u:pos){// Se revisa si alguno de los movimientos lleva a la victoria inmediatamente
        ...
        if(checkvec(v))return Winner[id]=player, Distance[id]=p, Move[id].push_back(u), void();  // Si el movimiento es ganador, se registra y se retorna
    ...
    for(auto &u:pos){
        ...
        if(Visited[new_id]==0)backtracking(p+1, v, new_id);  // Si el nuevo estado no ha sido visitado, se continúa el backtracking recursivamente
        if(Winner[new_id]==player)return Winner[id]=player, Distance[id]=Distance[new_id], Move[id].push_back(u), void();  // Si el movimiento conduce a victoria, se registra y retorna
...
int main(){
    ...
    cin>>boards;  // Pausa para mantener abierta la consola y revisar los resultados
