...
int boards=0;                       // Cantidad de estados del tablero jugados
map<long long, bool> Visited;       // Registra si un estado del tablero ya ha sido visitado
map<long long, int> Winner;         // Registra quién gana en un estado del tablero
...
void backtracking(int p, vector<int> v, long long id){// Función que revisa quién gana para un tablero dado usando backtracking
    Visited[id]=1;      // Marca el estado actual (id) como visitado
    boards++;           // Incrementa el contador de tableros evaluados
    
    int player=2-(p&1); // Determina el jugador actual: si la profundidad (p) es impar, es jugador 1; si es par, jugador 2
    vector<int> pos;    // Vector para almacenar las posiciones válidas donde se puede jugar

    for(auto &u:Order){// Recorre las casillas en el orden definido por la estrategia
        if(v[u-1]==0)pos.push_back(u-1);  // Si la casilla (u-1) está vacía, se añade a las posiciones válidas
    }

    if(pos.empty())return Winner[id]=2, Distance[id]=p, void(); // Si no hay movimientos válidos, se considera final de juego (empate favorable para jugador 2)
    
    for(auto &u:pos){// Revisa si algún movimiento inmediato produce victoria
        v[u]=player; // Realiza el movimiento asignando la casilla al jugador actual
        if(checkvec(v))return Winner[id]=player, Distance[id]=p, Move[id].push_back(u), void(); // Si el movimiento resulta ganador, se registra y retorna
        v[u]=0; // Revierte el movimiento si no produce victoria
    }

    for(auto &u:pos){  // Prueba cada movimiento válido de forma recursiva
        long long new_id=id+player*Pot_3[u]; // Calcula un nuevo ID para el estado resultante (codificación en base 3)
        v[u]=player; // Realiza el movimiento para el jugador actual
        if(Visited[new_id]==0)backtracking(p+1, v, new_id); // Si el nuevo estado no se ha visitado, se llama recursivamente
        if(Winner[new_id]==player)return Winner[id]=player, Distance[id]=Distance[new_id], Move[id].push_back(u), void(); // Si el movimiento lleva a victoria, se registra y retorna
        v[u]=0; // Revierte el movimiento
    }

    // Si no se encontró un movimiento ganador, se prolonga el juego al máximo
    Winner[id]=3-player; // Se asigna la victoria al contrincante (jugador opuesto)
    int max_distance=-1;
    for(auto u:pos){// Calcula la máxima distancia (número de jugadas) alcanzable entre los movimientos posibles
        long long new_id=id+player*Pot_3[u];
        max_distance=max(max_distance, Distance[new_id]);  // Se busca la mayor cantidad de jugadas posibles
    }
    
    Distance[id]=max_distance; // Se asigna al estado actual la máxima distancia obtenida
    
    for(auto &u:pos){// Se registran los movimientos que optimizan la distancia (que permiten alargar el juego)
        long long new_id=id+player*Pot_3[u];
...
void code(vector<int> v, int deep){// Función que revisa e imprime la Base de Datos generada desde una semilla 
    ...
    cout<<"Gana el jugador "<<Winner[id]<<"\nY se jugaron "<<boards<<" jugadas distintas\n"; // Muestra el resultado (ganador y cantidad de jugadas evaluadas)
    ...
    Winner.clear(); Distance.clear(); Visited.clear(); Move.clear(); boards=0; // Reinicializa todas las estructuras para procesar nuevas semillas
    return;
}
